...
│   ├── posts/                  # Blog posts
│   ├── snippets/               # Reusable content blocks for tag pages
│   └── img/                    # Media files (copied to dist/img/)
└── views/
    └── [theme-name]/           # Your theme folder
        ├── assets/             # Static assets
        │   ├── css/            # Stylesheets
        │   ├── js/             # JavaScript files
        │   └── tpl-img/        # Template images (icons, etc.)
        ...
        │   ├── article.phtml
        │   └── page-content.phtml
        ├── page.phtml          # Single page layout
        ├── post.phtml          # Single blog post layout
        ├── index.phtml         # Blog post list layout
        ├── tag.phtml           # Tag archive layout
        └── homepage.phtml      # Optional custom homepage template
```
...
return json_decode(json_encode([
    ...
    "slugs" => [
        // Blog index filename:
        // - "index" → blog on homepage (yoursite.com/index.html)
        // - "blog"  → separate blog page (yoursite.com/blog.html)
        "post_index" => "blog",
    ...
    // You can add any custom keys – they'll be available in templates
    // as $config->your_key
...
```
config/
├── config.php           # Production config (used by default)
├── local-config.php     # Local development
└── preview-config.php   # Staging/preview environment
```
...
<title><?= $config->title ?> – <?= $entity->get('title') ?? $config->tagline ?></title>
...
└── tpl-img/
    └── logo.svg        → dist/tpl-img/logo.svg
```
...
```
content/img/
├── photo.jpg           → dist/img/photo.jpg
└── diagram.png         → dist/img/diagram.png
```
...
✍️ **Write Content** → [Creating Content](03.creating-content.md)

🚀 **Deploy Automatically** → [GitHub Actions Deployment](04.github-actions-deployment.md)

📖 **API Details** → [Template API Reference](05.template-api-reference.md)
