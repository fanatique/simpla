...
That means you can basically run havoc and put any type of code into any template file, nothing will stop you! – Except from your inner code guru, who whispers things about separation of concerns and that mixing business logic and presentational logic is the root of all evil, right?

#### Themes

A Theme is a very fancy name for a sub folder inside the Views folder. But it does the trick! 😁
...
Whether or not a seamless switch between Themes is actually possible without adjusting the Content or other things – is something you can go and find out, if you feel the need to.
...
2. There is absolutely **no processing of stylesheets, scripts or images** happening. And that is sad. But this is a content generator and not a JavaScript build tool – and as much as some those features might come in handy, the costs in terms of dependencies and complexity are definitely beyond what would be acceptable for a tool like Simpla.
...
### Adding Snippets to Index Pages

Imagine that you have a collection of blog posts with a certain tag (in our case this is the tag `static-websites`). Simpla will automatically generate an index page for this tag, including all published posts, which are using that tag.

On this index pages it might be helpful to add a description, which clarifies, what this index is about. This can be a short text, maybe an image or even a video. Whatever it is, it will help your reader, as much as it will help search engines to make sense out of this collection.

In Simpla such a descriptive section is called a **Snippet** and it is actually nothing but a normal piece of content, that is rendered in place.

![A Tag Page with a Snippet](./tag_page_with_snippet.jpg)

If you want to add a Snippet to an Index Page, simply go to `page/content/snippets` and create a file with the name of the index you want to create it for. In our case that is the name of the Tag `static-websites.md`.

As with any other piece of content in Simpla, a snippet consists of meta information and a Markdown body.

So the content of our `static-websites` Snippet looks like this:
...
In the corresponding template (in this case `tag.phtml`), you'll find the following line:

```
  <?php if($snippet !== null) print $snippet->get('content') ?>
```

It checks, whether we have a Snippet for this index and if so, simply prints the generated HTML code. If that is too boring, feel free to grab the `$snippet` and render it into something more elaborate. You can easily create a Partial and include it, if you want, since `$snippet` is actually nothing but an [Entity](https://github.com/fanatique/simpla/blob/main/engine/src/Simpla/Entity/AbstractEntity.php) and therefore supports all methods en Entity would give you.
