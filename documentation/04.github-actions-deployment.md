...
```
┌─────────────────┐     ┌──────────────────┐     ┌─────────────────┐
│   Push to Git   │ ──► │  GitHub Actions  │ ──► │   Deploy Site   │
│                 │     │                  │     │                 │
│ • Edit content  │     │ • Build w/Docker │     │ • FTP/SFTP      │
│ • Push to main  │     │ • Generate HTML  │     │ • GitHub Pages  │
│                 │     │ • Run tests      │     │ • Netlify       │
└─────────────────┘     └──────────────────┘     └─────────────────┘
```
...
│   │   └── preview-config.php # Optional: staging config
│   ├── content/
│   └── views/
└── dist/                     # Generated (can be gitignored)
```
...
```
page/config/
├── config.php           # Default (production)
├── preview-config.php   # Staging/preview
└── local-config.php     # Local development (gitignored)
```
...
**Cause:** The Docker image was built on an ARM machine (e.g., Apple Silicon Mac) but GitHub Actions runs on AMD64/x86_64.

**Fix (for Simpla maintainers):** Build a multi-architecture image using `docker buildx`:
...
This creates a single image that works on both ARM (Apple Silicon) and AMD64 (GitHub Actions, most servers).
